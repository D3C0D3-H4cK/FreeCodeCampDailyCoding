...
def launch_fuel(payload):
    combustibleAcumulado = 0

    while True:
        masaTotal = payload + combustibleAcumulado
        nuevoCombustible = masaTotal / 5
        diferencia = nuevoCombustible - combustibleAcumulado

        if diferencia < 1:
            break

        combustibleAcumulado = nuevoCombustible

    return round(nuevoCombustible, 1)
